...
public class BlockFreljordCrop extends BlockCrops {
	@SideOnly(Side.CLIENT)
	private Icon[] iconArray;

	public BlockFreljordCrop(int par1) {
		super(par1);
	...
	public int getSeedItem() {
		return LeagueItems.freljordSeeds.itemID;
	}
	...
	public int getCropItem() {
		return LeagueItems.freljordBerry.itemID;
	}
	...
	public ArrayList<ItemStack> getBlockDropped(World world, int x, int y, int z, int metadata, int fortune) {
		...
		if (metadata >= 7) {
			for (int n = 0; n < 2 + fortune; n++) {
				if (world.rand.nextInt(25) <= metadata) {
	...
	public void registerIcons(IconRegister par1IconRegister) {
		...
		for (int i = 0; i < this.iconArray.length; ++i) {
			this.iconArray[i] = par1IconRegister.registerIcon("league:freljordCrop" + i);
		}
	}
}
