...
import leagueofcrafters.entity.damagesources.EntityKnifeDamageSource;
...
public class EntityKnife extends Entity implements IProjectile {
	...
	/** 1 if the player can pick up the Knife */
	public int canBePickedUp;

	/** Seems to be some sort of timer for animating an Knife. */
	...
	/** The amount of knockback an Knife applies when it hits a mob. */
	...
	/**
	 * Similar to setKnifeHeading, it's point the throwable entity to a x, y, z
	 * direction.
	 */
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (this.shootingEntity == null) {
						damagesource = this.causeKnifeDamage(this, this);
					} else {
						damagesource = this.causeKnifeDamage(this, this.shootingEntity);
					}
	...
	public static DamageSource causeKnifeDamage(EntityKnife par0EntityKnife, Entity par1Entity) {
		return (new EntityKnifeDamageSource("Knife", par0EntityKnife, par1Entity)).setProjectile();
	}
	...
	public void onCollideWithPlayer(EntityPlayer par1EntityPlayer) {
		if (this.inGround && this.KnifeShake <= 0) {
			...
			if (!this.inGround && par1EntityPlayer != this.shootingEntity) {
				par1EntityPlayer.addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 100, 1));
			} else {
				return;
			}
	}
	...
	/**
	 * Sets the amount of knockback the Knife applies when it hits a mob.
	 */
	...
	/**
	 * Whether the Knife has a stream of critical hit particles flying behind
	 * it.
	 */
